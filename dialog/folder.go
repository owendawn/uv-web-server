...
func (f *folderDialog) setOpened(file *folderDialogItem) {
	if f.selected != nil {
		f.selected.isCurrent = false
		f.selected.Refresh()
	}
...
func NewFileOpen(callback func(fyne.URIReadCloser, error), parent fyne.Window) *FolderDialog {
...
// The dialog will appear over the window specified when Show() is called.
func NewFileSave(callback func(fyne.URIWriteCloser, error), parent fyne.Window) *FolderDialog {
	dialog := &FolderDialog{callback: callback, parent: parent /*, save: true*/}
	return dialog
}
