...
func dirList(w http.ResponseWriter, r *http.Request, f File) {
	...
	fmt.Fprintf(w, "<style>td{border-top:1px dotted;padding:5px;}a{text-decoration:none;}</style><table><thead><tr><th>FileName</th></tr></thead><tbody>\n")
	fmt.Fprintf(w, "<tr><td><a href=\"%s\">%s</a></td></tr>\n", "../", "../")
	for _, d := range dirs {
		name := d.Name()
		var isFile = true
		if d.IsDir() {
		...
		if isFile {
			fmt.Fprintf(w, "<tr><td><a href=\"%s\" download=\"%s\">%s</a></td></tr>\n", url.String(), htmlReplacer.Replace(name), htmlReplacer.Replace(name))
		} else {
